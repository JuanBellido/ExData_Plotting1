plot4<-function()
{
        ...
        par(mfrow=c(2,2))
        with(dtInput,plot(Time,Global_active_power
                                ,type="l"
                                ,xlab=NA
        ...
        with(dtInput,plot(Time,Voltage
                          ,type="l"
                          ,xlab="datetime"
                          ,ylab="Volatage"
                         )
        ...
        legend(x="topright", legend=names(dtInput)[7:9],col=c('black','red','blue'),lty=1,bty='n')
        
        
        with(dtInput,plot(Time,Global_reactive_power
                          ,type="l"
                          ,xlab="datetime"
                          )
                )
